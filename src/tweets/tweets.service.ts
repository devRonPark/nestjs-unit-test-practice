...
export class TweetsService {
  ...
  updateTweet(tweet: string, id: number) {
    ...
    if (!tweetToUpdate) throw new Error(`This Tweet does not exist`);
    if (tweet.length > 100) throw new Error(`Tweet too long`);
  ...
  deleteTweet(id: number) {
    ...
    if (!tweetToDelete) throw new Error(`This Tweet does not exist`);
